...
def get_successors(state, parent_state, step):
    """Generate successors in format [state, step_added, parent_state]"""
    successors = []
    n = len(state)
...
def bfs_trace(start_state, goal_state):
    ...
    while q:
        ...
        # Append current state to visited after popping
        visited.append([current, step_added, parent_state])

        # Build frontier for printing
        ...
        # Generate successors and append all to queue
...
solution_path = bfs_trace(start_state, goal_state)
